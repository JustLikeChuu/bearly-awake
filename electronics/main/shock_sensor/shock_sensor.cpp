...
bool readShock(int shockIn){
    shockVal = digitalRead (shockIn); // read digital interface is assigned a value of 3 val
    if (shockVal == HIGH) // When the shock sensor detects a signal, LED flashes
    {
      return true;
    }
    else
